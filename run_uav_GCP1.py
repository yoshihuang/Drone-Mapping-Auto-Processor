...
LANG = {
    "en": {
        ...
        "mode_high": "🏗️ High Precision - Detailed DSM & 3D model (< 300 imgs)",
    ...
    "zh": {
        ...
        "mode_high": "🏗️ 高精度重建模式 (High Precision) - 產出細緻 3D (<300張)",
        "mode_huge": "🗺️ 超大範圍切割模式 (Huge Dataset) - GCP 防崩潰穩定版",
        "btn_start": "🚀 開始全自動處理",
        "btn_init": "⚙️ 系統初始化中...",
        "time_elapsed": "⏳ 經過時間：",
        "log_lbl": "📜 執行監控 (Log)：",
...
class DroneProcessingApp:
    ...
    def show_gcp_help(self):
        ...
        def download_template():
            ...
            if filepath:
                template_content = "WGS84 EPSG:4326\n121.564\t25.033\t10.5\t1500\t2000\tIMG_001.JPG\tGCP_1\n121.565\t25.034\t11.2\t1800\t2500\tIMG_002.JPG\tGCP_2\n"
                try:
    ...
    def run_odm(self):
        ...
        if mode == "fast":
            ...
            odm_params.extend([
                ...
                "--pc-quality", "low",       # 🛡️ 修復 OpenMVS 崩潰：調升品質，避免點雲過少
                "--dem-resolution", "10",
                "--max-concurrency", "2",
                "--force-gps",               # 🛡️ 修復分塊迷失：結合 GCP 運算時強制先做 GPS 對齊
                "--crop", "15"
                # 🛡️ 移除 --pc-filter：避免 GCP 拉扯點雲時觸發雜訊誤判導致 0 points
            ])
