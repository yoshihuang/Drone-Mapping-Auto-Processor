...
import shutil
import locale
import ctypes  # 🌟 新增：呼叫 Windows 底層 API
import tkinter as tk
from tkinter import filedialog, messagebox, scrolledtext
from tkinter import ttk 

# ==========================================
# 🌐 國際化語言字典 (Translation Dictionary)
# ==========================================
LANG = {
    "en": {
...
class DroneProcessingApp:
    def __init__(self, root):
        ...
        # 🌟 核心修正：100% 精準的 Windows 系統語言偵測
        self.lang_code = "en" # 預設為英文
        try:
            if os.name == 'nt':
                # 呼叫 Windows 核心 API 取得語言 ID (LCID)
                lang_id = ctypes.windll.kernel32.GetUserDefaultUILanguage()
                # 1028(台灣), 2052(中國), 3076(香港), 4100(新加坡), 5124(澳門)
                if lang_id in [1028, 2052, 3076, 4100, 5124]:
                    self.lang_code = "zh"
            else:
                # 如果是 Mac 或 Linux 系統，使用原本的字串檢查法
    ...
    def run_odm(self):
        ...
        if mode == "fast":
            ...
            command.extend(["--pc-quality", "medium", "--dem-resolution", "5", "--max-concurrency", "4"])
        elif mode == "huge":
            command.extend([
                "--split", "150", "--split-overlap", "15", 
                "--feature-quality", "low", "--min-num-features", "4000", 
                "--pc-quality", "low", "--dem-resolution", "10", "--max-concurrency", "4"
            ])
